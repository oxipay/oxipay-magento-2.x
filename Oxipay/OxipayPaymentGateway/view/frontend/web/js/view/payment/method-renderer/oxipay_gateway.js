    ...
    function ($, Component, ko, url, Quote, globalMessageList) {
        ...
        return Component.extend({
            ...
            updateData: function(response) {
                ...
                if(paymentMethod && paymentMethod.method && paymentMethod.method == self.getCode()){
                    ...
                    self._requestTimeout = window.setTimeout(function() {
                        ...
                        $.post('/oxipay/Checkout/RequestDetails', quoteData)
                            .complete(function(response) {
                                ...
                                if(!responseData.x_customer_email
                                    ...
                                    //suppress error message when unchecking billing address is the same option
                                    if(billingAddress){
                                        globalMessageList.addErrorMessage({'message': 'Please enter your billing details'});
                                    }
                                    self.canSubmit(false);
                                }
                                else if(!responseData.x_customer_billing_country
                                    || allowedCountriesArray.indexOf(responseData.x_customer_billing_country) == -1
                                    || !responseData.x_customer_shipping_country
                                    || allowedCountriesArray.indexOf(responseData.x_customer_shipping_country) == -1
                                ){
                                    globalMessageList.addErrorMessage({'message': 'Orders from this country are not supported by Oxipay. Please select a different payment option.'});
                                    self.canSubmit(false);
                                }
                                else{
                                    self.canSubmit(true);
